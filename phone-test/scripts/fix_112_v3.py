...
# Step 1: 后台启动下载
print("=== 启动后台下载Fluidd ===")
run('nohup bash -c \'cd /tmp && rm -f fluidd.zip && curl -L --connect-timeout 30 --max-time 300 https://github.com/fluidd-core/fluidd/releases/latest/download/fluidd.zip -o fluidd.zip && echo DONE > /tmp/fluidd_download_status || echo FAIL > /tmp/fluidd_download_status\' > /tmp/fluidd_download.log 2>&1 &')
print("下载已在后台启动，等待完成...")

# Step 2: 轮询等待下载完成
for i in range(60):
    time.sleep(10)
    out = run('cat /tmp/fluidd_download_status 2>/dev/null || echo PENDING', timeout=10)
    status = out.strip()
    print(f"  [{(i+1)*10}s] 状态: {status}")
    if status == 'DONE':
        break
    if status == 'FAIL':
        print("下载失败!")
...
# Step 3: 检查文件
out = run('ls -la /tmp/fluidd.zip 2>&1')
print(f"\n文件: {out.strip()}")
...
# Step 7: 修复usb-watchdog v4 + autosuspend
