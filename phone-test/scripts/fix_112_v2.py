...
# 1. 下载Fluidd前端
print("=== 下载Fluidd前端 ===")
out = run('cd /tmp && rm -f fluidd.zip && curl -L --connect-timeout 30 --max-time 180 https://github.com/fluidd-core/fluidd/releases/latest/download/fluidd.zip -o fluidd.zip 2>&1 && echo DOWNLOAD_OK || echo DOWNLOAD_FAIL', timeout=200)
print(f"下载结果: {out.strip()[-50:]}")

# 检查文件大小
out = run('ls -la /tmp/fluidd.zip 2>&1')
print(f"文件: {out.strip()}")
...
# 5. 修复usb-watchdog v4 + autosuspend
